...
logger = get_logger(name='main', log_level=logging.DEBUG, log_to_file=True)
...
def main():
    ...
    for file in file_list_dir:
        ...
        df['Subcategory'] = parallel_apply_process(df, transformer.particular_scrapper, 'Particulars', max_workers=8)
        if 'Subcategory' in df.columns:
            logger.info(f'Processing Transformation on `CATEGORY`......')
            df['Category'] = parallel_apply_process(df, transformer.category_mapper, 'Subcategory', max_workers=8)
    ...
    if delete_temp_dir:
        logger.warning(f'Deleted Temporary directory')
