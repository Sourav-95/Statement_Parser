...
class DB_DeltaHandler:
    ...
    def _validate_table_schema(self, table_name: str, expected_columns: set):
        ...
        # logger.debug(f'Columns from DB Table: {actual_columns}')
        # logger.debug(f'Columns from Expected Table: {expected_columns}')
    ...
    def load_delta(self, parsed_df: pd.DataFrame, expected_columns:set, bank_name: str, target_table: str):
        ...
        if last_date:
            ...
            logger.debug(f'Deleting the existing records for last date')
            with self.engine.begin() as conn:
                delete_stmt = text(f"DELETE FROM {target_table} WHERE Date = :last_date")
                conn.execute(delete_stmt, {"last_date": last_date})
